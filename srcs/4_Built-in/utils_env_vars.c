...
/*   Updated: 2026/03/18 12:34:13 by vnaoussi         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	add_env_var(t_env_var **head, char *key, char *value)
{
	...
	if (!head || !key || check_if_exit_and_replace(*head, key, value))
		return ;
	...
	if (value)
		...
		node->value = NULL;
	if (ft_strcmp(previous_node->key, key) >= 0 || !*head)
	{
		node->next = previous_node;
		*head = node;
		return ;
	}
	while (previous_node->next && ft_strcmp(previous_node->next->key, key) < 0)
